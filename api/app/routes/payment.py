...
async def pick_best_server() -> Server:
    servers = await Server.filter(is_active=True).order_by("current_clients", "id").all()
    if not servers:
        raise HTTPException(status_code=503, detail="Нет доступных серверов")
    return servers[0]


async def create_remote_client(server: Server, email: str, client_uuid: str,
                               expiry_ms: int, flow: str = "xtls-rprx-vision") -> None:
    client = XuiClient(
...
async def issue_subscription(user: User, plan_id: str) -> Subscription:
    ...
    existing = await Subscription.filter(user_id=user.id, is_active=True).order_by("-expires_at").first()
    if existing:
        server = await Server.get_or_none(id=existing.server_id)
        if not server:
            server = await pick_best_server()
            existing.server_id = server.id
            existing.client_uuid = generate_uuid()

        # Extend expiry by duration days from current expiry (or now if expired)
        base = existing.expires_at if existing.expires_at and existing.expires_at > now else now
        if base.tzinfo is None:
            base = base.replace(tzinfo=timezone.utc)
        new_expires = base + timedelta(days=duration)
        existing.expires_at = new_expires
        existing.devices = plan["devices"]
        existing.duration_days = existing.duration_days + duration
        existing.is_active = True

        # Update remote client expiry
        safe = server.name.replace(" ", "_").replace("/", "_")[:20]
        email = f"cwim_{safe}_{user.id}"
        existing.client_email = email
        expiry_ms = int(new_expires.timestamp() * 1000)
        flow = server.config_flow or "xtls-rprx-vision"

        await create_remote_client(server, email, existing.client_uuid, expiry_ms, flow=flow)
        await existing.save()
        server.current_clients += 1
        await server.save()
        return existing

    # New subscription — pick best server
    server = await pick_best_server()
    safe = server.name.replace(" ", "_").replace("/", "_")[:20]
    email_tag = f"cwim_{safe}_{user.id}"
    client_uuid = generate_uuid()
    flow = server.config_flow or "xtls-rprx-vision"
    expires_at = now + timedelta(days=duration)
    expiry_ms = int(expires_at.timestamp() * 1000)

    await create_remote_client(server, email_tag, client_uuid, expiry_ms, flow=flow)
...
@router.post("/create")
async def create_payment(body: CreatePaymentRequest, user: User = Depends(get_current_user)):
    ...
    try:
        ...
        await Transaction.filter(id=txn.id).update(status="completed", paid_at=datetime.now(timezone.utc))
