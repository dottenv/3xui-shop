...
const descMap = { personal: null, security: null, language: null, theme: null }
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4 bg-surface border border-border rounded-2xl p-5">
        ...
        <div className="min-w-0 flex-1">
          <p className="font-bold truncate">{user?.first_name || user?.email?.split('@')[0] || t('app.pages.settings.personal')}</p>
        ...
        {sections.map((s, i) => (
          <Link key={i} to={s.to} className="flex items-center gap-4 px-5 py-4 hover:bg-bg/50 transition-colors">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-medium text-sm">{t('app.pages.settings.' + s.key)}</p>
              <p className="text-xs text-muted mt-0.5">{s.key === 'language' ? langNames[lang] || lang : ''}</p>
            </div>
